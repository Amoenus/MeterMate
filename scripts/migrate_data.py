...
class MeterMateMigrator:
    ...
    def _migrate_reading(
        self, entity_id: str, reading: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if (
            "unit" in migrated_reading
            and ATTR_UNIT_OF_MEASUREMENT not in migrated_reading
        ):
            ...
            updated = True
            logger.debug(
                "Updated field name for reading %s", reading.get("id", "unknown")
            )
        ...
        if new_unit:
            ...
            updated = True
            logger.info(
                "Corrected unit for %s reading %s: %s -> %s",
                entity_id,
                reading.get("id", "unknown"),
                current_unit,
                new_unit,
            )
    ...
    def migrate(self) -> None:
        ...
        for entity_id, readings in readings_data.items():
            ...
            if not isinstance(readings, list):
                logger.warning(
                    f"Unexpected data format for entity {entity_id}, skipping"
                )
    ...
    def _print_statistics(self) -> None:
        ...
        logger.info(
            f"  Field name updates (unit -> unit_of_measurement): {self.stats['field_updates']}"
        )
        logger.info(
            f"  Unit corrections (kWh -> L for water meters): {self.stats['unit_corrections']}"
        )
...
def main():
    ...
    try:
        migrator = MeterMateMigrator(config_path=args.config_path, dry_run=args.dry_run)
        migrator.migrate()
